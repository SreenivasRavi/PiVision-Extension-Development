...
var sunburst = Kapsule({
  ...
  methods: {
    _parseData: function _parseData(state) {
      if (state.data) {
        ...
        var hierData = d3Hierarchy.hierarchy(state.data, accessorFn(state.children)).sum(accessorFn(state.size)).sort(function (a, b) {
  ...
  init: function init(domNode, state) {
    ...
    // tooltips
    state.tooltip = d3Selection.select('body').append('div').attr('class', 'sunburst-tooltip');
    ...
    state.canvas.on('mousemove', function () {
      state.tooltip.style('left', d3Selection.event.pageX + 'px').style('top', d3Selection.event.pageY + 'px');
    });
  ...
  update: function update(state) {
    ...
    var transition$$1 = d3Transition.transition().duration(TRANSITION_DURATION);
    ...
    state.svg.transition(transition$$1).tween('scale', function () {
      ...
      var yd = d3Interpolate.interpolate(state.radiusScale.domain(), [focusD.y0, 1]);
      return function (t) {
